...
- `ghcr.io/rocker-org/devcontainer/tidyverse` as the FROM line — `:4.6` (R 4.6.1 on ubuntu:noble since v1.1.0), pinned by SHA digest (the tag floats as Rocker patches; the digest freezes us at a known build; bumping it is a deliberate commit, and an R-version smoke test fails the build if a bump changes the R minor version unnoticed).
...
- **vscode-R session-watcher shim for R ≥ 4.6 (since v1.1.2):** R 4.6 changed startup to no longer call a globalenv `.First.sys` override — the exact mechanism vscode-R's session watcher uses to attach — so student `plot()` silently fell back to `Rplots.pdf` (no error anywhere; entered with the R 4.6 bump in v1.1.0; arf was wrongly suspected first and is exonerated — plain R 4.6.1 reproduces it). The fix is a `.First` shim appended to `Rprofile.site` that fires an armed shadow, plus a smoke test (`arf headless` harness — the TUI can't run in a build and `arf -e` skips full startup) proven red-without/green-with. Remove the shim when vscode-R ships its own R-4.6 fix. If plots ever regress to PDFs again, check this contract FIRST, and check what R version changed.
...
- Modeling stack from a DATED P3M snapshot (`P3M_SNAPSHOT` build arg since v1.1.0 — reproducible rebuilds; bump the date to refresh these stacks): `tidymodels`, `bonsai`, engines (`xgboost`, `lightgbm`, `randomForest`, `ranger`, `glmnet`), `catboost` (pinned GitHub release binary, amd64), `brms` (+ explicit `BH`/`RcppEigen`/`RcppParallel` — rstan binary installs skip its LinkingTo deps, but runtime Stan compilation needs them).
