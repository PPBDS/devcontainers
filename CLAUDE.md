...
- GitHub CLI (`gh`); Quarto (pinned ARG); `arf` (Rust R console, pinned ARG, installed as rstudio, symlinked to `/usr/local/bin/arf` for consumer config stability). **arf pin ROLLED BACK 0.4.5 → 0.3.4 in v1.1.2:** arf 0.4.x resolves `.First.sys` in the base namespace at startup, bypassing the globalenv shadow the vscode-R session watcher installs — student `plot()` calls silently fell back to `Rplots.pdf` (diagnosed 2026-08-12). A startup smoke test right after the arf install now guards this contract (verified to fail on 0.4.5). Do not re-bump arf until that test passes against the new version — watch the upstream report on eitsupi/arf for the fix.
