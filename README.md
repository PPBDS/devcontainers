...
- **R** — the rocker/tidyverse foundation, plus the PPBDS course packages (`tutorial.helpers`, `vscode.tutorials`), the `arf` console, Quarto, and `httpgd`.
- **Python** — a basic data-science stack installed with [`uv`](https://docs.astral.sh/uv/) into `/opt/venv` from a pinned lockfile (`student/requirements.lock`): numpy, pandas, matplotlib, seaborn, scikit-learn, statsmodels, jupyter, ipykernel. It's the default `python` on `PATH` (and R's reticulate interpreter), with a registered Jupyter kernel — so students can use Python in VS Code notebooks or in Quarto `.qmd` chunks alongside R. The venv is group-writable, so students can `pip install` more packages.
